        ...
        static public bool ExportDVTables( LithoData data, string FolderName, string ModelName )
        {
            ...
            try
            {
                ...
                foreach (var l in data.Layers)
                {
                    ...
                    //this reads the table from th UI.
                    ...
                    */
                    //we changed it so we actually regenerate the table from the data stored in the layer
                    List<KeyValuePair<double, double>> dvt = l.GetDVTTable();
                    foreach (var p in dvt)
                        lines.Add(p.Key.ToString() + "\t" + p.Value.ToString());
                    lines.Add(" ");

                    if (counter == 1)
                        header = header.Replace("NPOINTS", dvt.Count().ToString());
            ...
            try
            {
                ...
                using (IProgress nestedProgressBar = PetrelLogger.NewProgress(0, 100, ProgressType.Cancelable, Cursors.WaitCursor))
                {
                    ...
                    foreach (string filePath in filePaths)
                    {
                        ...
                        {
                            File.Copy(filePath, mshFile, true);
                        }
