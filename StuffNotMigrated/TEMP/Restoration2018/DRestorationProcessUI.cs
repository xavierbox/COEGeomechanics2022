...
namespace Restoration
{
    ...
    partial class DRestorationProcessUI : UserControl
    {
        ...
        private Dictionary<string, float[]> _dynelFile;
        private DRestorationProcess _process;

        private Font greyFont;//= new gridPresentationBox.Font;
        private Font blackFont;// = new gridPresentationBox.Font;

        private List<Borehole> connectedWells;
        private List<Borehole> changedWells;
        ...
        private float[] GetEquivalentPlasticStrainScalarFromTensor( Dictionary<string, float[]> tensor )
        {
        ...
        private void ConnectCalculationEvents()
        {
            ...
            #endregion predict fractures from stress in grid or points
            ...
            this.executeWellCalculationsButton.Click += ( sender, evt ) =>
         {
            ...
            #endregion well-data-comparison

        }

        #endregion KeyEvents Triggering Calculations
        ...
        private WellFractureDataModel GetWellObservedData()
        {
            ...
            foreach (Borehole well in wells)
            {
                ...
                foreach (var sample in sortedSamples)
                {
                    ...
                    Point3 Location = BoreholeTools.getPositionAtWellMD(well, md);
        ...
        private List<FracturePatch> _patches;

        private List<FracturePatch> Patches
        {
            get
            {
                return _patches;
                ...
                using (IProgress p = PetrelLogger.NewProgress(0, 100))
                {
                    ...
                    foreach (string wellName in data.WellFractures.Keys)
                    {
                        ...
                        BoreholeTools.SetPointWellLogMDs(xx, mds);
                        {
                            ...
                            BoreholeTools.SetPointWellLogAttribute(xx, "Dip angle", dip.ToList(), PetrelProject.WellKnownTemplates.GeometricalGroup.DipAngle);
                            BoreholeTools.SetPointWellLogAttribute(xx, "Dip azimuth", dipAzimuth.ToList(), PetrelProject.WellKnownTemplates.GeometricalGroup.DipAzimuth);
                        }
                        {
                            ...
                            BoreholeTools.SetPointWellLogAttribute(xx, "CostMatchedType", typeMatchedCost.ToList(), PetrelProject.WellKnownTemplates.MiscellaneousGroup.General);
        ...
        #endregion map to grid

        #endregion DisplayControllerResults
        ...
        private Dictionary<string, float[]> GetStressFromGridPropertyFolder( PropertyCollection col )
        {
        ...
        private Dictionary<string, float[]> GetTensorFromPropertyFolder( PropertyCollection col, string tensorName, Template t )
        {
            Dictionary<string, float[]> names = new Dictionary<string, float[]>()
                    {
                    { tensorName+"XX",null},
        ...
        private List<Borehole> SelectedWells
        {
            get
            {
        ...
        private RegularHeightFieldSurface SelectedHorizon
        {
            get
            {
        ...
        private void DisplayInPetrelWellIntersectedFractures( Dictionary<Borehole, List<Tuple<FracturePatch, double>>> intersections, string name = "" )
        {
        ...
        private Dictionary<Borehole, List<Tuple<FracturePatch, double>>> IntersectFracturesAndWells( List<Borehole> wells, FractureNetwork dfn, List<int> setIndicesSelected = null )
        {
