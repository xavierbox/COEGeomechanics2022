...
namespace Restoration.Controllers
{
       ...
       public class FracturePredictor
        ...
        public static FractureModel PredictFracturesFromStress(Dictionary<string, float[]> data, double fangle = 30.0, Vector3 [] centers = null, bool filterOutlayers=true)
        {
            ...
            //public static FractureModel PredictFractures(Dictionary<string, List<double>> data, Vector3 traslation, double fangle = 30.0)
            //{
            //    Random rand = new Random();
            ...
            //        for (int nshear = 0; nshear < 2; nshear++)
            //        {
            //            double fangleRad = fangle * Math.PI / 180;
            //            double coefFang = Math.Tan(fangleRad);
            //            Matrix3x3 rotateShearFracture = stressTensor * rotationShearFracture((sign * fangleRad));
            ...
            //        //                    int index = fracs.Count() - 1 - rand.Next(0, 2);
            //        //                   fracs.Add(new Fracture(fracs[index]));

            //        int index = shear.Count() - 1 - rand.Next(0, 2);
            //        shear.Add(new Fracture(shear[index]));
            ...
            //    //getNormalize(fracs);
            //    FractureModel model = new FractureModel();
            //    model.Fractures.AddRange(Translate(traslation, normalizedIntensity(shear)  ));
            //    model.Fractures.AddRange(Translate(traslation, normalizedIntensity(joints)  ));
            //    model.Fractures.AddRange(Translate(traslation, normalizedIntensity(stylolite)  ));

            //    return model;
            //}


        static List<Fracture> Translate(Vector3 dir, List<Fracture> fracs)
        {
            //return null;
            ...
            static List<Fracture> normalizedIntensity(List<Fracture> fracs, bool filterOutlayers = true )
            {
    ...
    public class FracturePredictor
    {
        ...
        /// \endcode
        /// </summary>
        static List<KeyValuePair<double, Vector3>>  EigenVectors(double[] a)
        {
...
static List<KeyValuePair<double, Vector3>> EigenVectors(double sxx, double sxy, double syy, double sxz, double syz, double szz)
        ...
        public static FractureModel PredictFractures(Dictionary<string, List<double>> data, double fangle = 30.0 )
        {
            ...
            List<Fracture> joints   = new List<Fracture>();
            List<Fracture> styolite = new List<Fracture>();
            List<Fracture> shear    = new List<Fracture>();
...
//this is a styolite maximum = intensity 
                ...
                shear.Add(new Fracture(fracs[index]));


            }//i

            FractureModel model = new FractureModel();
            model.Fractures.AddRange( normalizedIntensity( shear) );
            model.Fractures.AddRange( normalizedIntensity( joints) );
            model.Fractures.AddRange( normalizedIntensity( styolite) );
        ...
        static List<Fracture> normalizedIntensity( List<Fracture> fracs )
        {
            var min = fracs.Select(t => t.Intensity).Min();
            var max = fracs.Select(t => t.Intensity).Max();
            return fracs.Select( t => { t.Intensity = 4.0; return t;} ).ToList(); 
        }
