...
namespace Restoration
{
    ...
    partial class ExportGeometryToDynelUI : UserControl
    {
        ...
        Dictionary<string, IIndexedTriangleMesh> HorizonMeshes { get; set; }


        Dictionary<string, IIndexedTriangleMesh> FaultMeshes { get; set; }
        ...
        public void ConnectEvents()
        {
            this.dropFaults.DragDrop += (sender, evt) =>
            {
                ...
                //else if (dropped as Slb.Ocean.Petrel.DomainObject.StructuralGeology.FaultFramework != null)
                //{
                //Slb.Ocean.Petrel.DomainObject.StructuralGeology.FaultFramework framework = dropped as Slb.Ocean.Petrel.DomainObject.StructuralGeology.FaultFramework;
                //foreach (var f in framework.InitialFaults)
                //    FaultMeshes.Add(f.IndexedTriangleMesh);

                //faultsPresentationBox.Text = framework.Layer;
                //faultsPresentationBox.Image = PetrelImages.FaultModel;

                //model = framework.StructuralFramework.FaultFramework..StructuralModel;
                //}
                ...
                else if (dropped as Slb.Ocean.Petrel.DomainObject.StructuralGeology.StructuralModel != null)
                {
                    ...
                    //CheckWhatIHappening(model);

                }

                else
                {
                    faultsPresentationBox.Text = "Please drop valid data: Structural model, Final Fault Model or Horizons Collection";
                    faultsPresentationBox.Image = null;

                    MessageBox.Show("Please drop valid data: Structural model, Final Fault Model or Horizons Collection", "Missing data", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        private void CheckWhatIHappening(Slb.Ocean.Petrel.DomainObject.StructuralGeology.StructuralModel model)
        {
            var initFaultsCollection = model.StructuralFramework.FaultFramework.InitialFaults;
            var initBorda = initFaultsCollection.Where(w => w.Name == "BordaJab").ToArray()[0];
            var mesh1 = initBorda.IndexedTriangleMesh;
            int nPoints = mesh1.VertexCount;
            int nTriangles = mesh1.TriangleCount;
            ExportAsGoCad("InitialFaultBorda", "C:\\tst", mesh1);


            var faultCollection = model.FinalFaultCollection.FinalFaults;

            var finalBorda = faultCollection.Where(t => t.Name == "BordaJab").ToArray()[0];
            var mesh2 = finalBorda.IndexedTriangleMesh;
            int nPoints2 = mesh2.VertexCount;
            int nTriangles2 = mesh2.TriangleCount;
        ...
        //private void FixTruncations(IIndexedTriangleMesh mesh, ref List<Vector3> vertices, ref List<IndexedTriangle> triangles)
        //{
        //    FixVertexTruncations(mesh, ref vertices, ref triangles );
        //}
        ...
        private void exportButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Error when exporting:\n " + exc.ToString(), "Data was not exported", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
            }
