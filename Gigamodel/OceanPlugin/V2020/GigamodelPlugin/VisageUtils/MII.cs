...
namespace Gigamodel.VisageUtils
{
    ...
    public class AsteriskInstruction
    {
        public AsteriskInstruction( string name = "", string value = "", string preffix = "*" )
        {
            Value = value;
            Preffix = preffix;
            Name = name;
    ...
    public class MII : MIIBase
    {
        ...
        /// <summary>
        /// //////////////////////////////////////////////////////////////////
        public void UpdateSwitches( Dictionary<string, List<KeyValuePair<string, bool>>> options )
        {
            foreach (string asteriskName in options.Keys)
            {
                List<KeyValuePair<string, bool>> switches = options[asteriskName];

                foreach (KeyValuePair<string, bool> s in switches)
                {
                    SetOrReplaceHash(asteriskName, s.Key, s.Value == true ? "1" : "0");// s.Value.ToString());
                }
            }
        }

        //public MIIOLD(string ver, string modelName = "Default", int debugLevel = 0) : base()
        //{
        //    Version = ver;
        ...
        public MII( string ver, string modelName = "Default", int debugLevel = 0 ) : base()
        {
            Version = ver;
            StreamKeywordEnabled = true;
            _modelName = modelName;
            ...
            ConfigureModelNameSection();
            ConfigureDebugSection();
            ConfigureElasticPlasticSection();

            Commands.Add(new AsteriskKey("TIDY", "", "*"));
            Commands.Add(new AsteriskKey("PRINTARRAYSIZES", "", "*"));
            Commands.Add(new AsteriskKey("PRINTTIMES", "", "*"));

            ConfigureHeaderSection();
            ConfigureGravity();
            ConfigureIncrements();

            ConfigureViscoplasticMethod();
        ...
        private void ConfigureModelNameSection()
        {
            Commands.Add(new AsteriskKey("MODELNAME", _modelName, "*"));
        }

        private void ConfigureDebugSection( int debugLevel = 0 )
        {
            if (debugLevel == 3)
            {
                //  *NOECHO
                ...
                Commands.Add(new AsteriskKey("NOECHO", "", "*", new Dictionary<string, string>()
                {  { "smaterials", "1" },
                   { "scgm"   , "-100" },
                   { "sloads" , "1" },
                   { "sele"   , "1" },
        ...
        private void ConfigureElasticPlasticSection()
        {
            Commands.Add(new AsteriskKey("ELASTIC", "", "*"));
        }
        ...
        private void ConfigureIncrements()
        {
            Commands.Add(new AsteriskKey("INCREMENTS,S", "1     1", "*"));
        }

        private void ConfigureViscoplasticMethod()
        {
            Commands.Add(new AsteriskKey("VISCOPLASTICMETHOD,S", "1.000000000E+000    1.000000000E-003", "*"));
        }
