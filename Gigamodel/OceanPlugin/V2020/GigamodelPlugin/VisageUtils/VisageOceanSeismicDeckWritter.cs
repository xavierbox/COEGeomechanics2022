...
namespace Gigamodel.VisageUtils
{
    public class VisageOceanSeismicDeckWritter
    {
        ...
        {
            string dataFolder = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GigaModel\\"); // GigaModelDataDefinitions.DataExportFolder;
            System.IO.Directory.CreateDirectory(dataFolder);

            //var v1 = GigaModelDataDefinitions.DataExportFolder; //Path.Combine( Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "GigaModel\\");
            //var v2 = GigaModelDataDefinitions.SimExportFolder;// SimExportFolder = System.IO.Path.Combine(DataExportFolder, "\\Simulations");
        ...
        public static string WriteMatFiles( MaterialsModelItem mat, string simulationName, string matsFolder )//, ref List<string> filesToInclude)
        {
            ...
            //gravity(1-phi)*density*0.001 (KPa)
            float[] scallingToVisageUnits = new float[] { 10.0f * (1.0f - 0.2f) * 0.001f * 0.1f * 10.0f, 0.001f, 1.0f };
        ...
        public static float[] GetEdgeLoad( Object Datum, bool offshore, float gapDensity, float seaWaterDensity, SeismicCube dens )
        {
            //to visage
            gapDensity *= (1000.00f);
            seaWaterDensity *= (1000.00f);

            float[] values = new float[dens.NumSamplesIJK.I * dens.NumSamplesIJK.J];
            int counter = 0;
            float offshoreFlag = offshore == true ? 1.0f : 0.0f;

            Point3 p1 = dens.PositionAtIndex(new IndexDouble3(0, 0, 1));
            Point3 p2 = dens.PositionAtIndex(new IndexDouble3(0, 0, 2));
            float halfCellHeight = (float)(0.5 * Math.Abs(p2.Z - p1.Z));

            for (int i = 0; i < dens.NumSamplesIJK.I; i++)
            {
                for (int j = 0; j < dens.NumSamplesIJK.J; j++)
                {
                    for (int k = 0; k < dens.NumSamplesIJK.K; k++)
                    {
                        if (k == 0)
                        {
                            ...
                            {
                                float distanceToDatum = SeismicCubesUtilities.DistanceToSurface(p, Datum) - halfCellHeight;
                                float overburden = distanceToDatum < 0.0f ? 0.0f : gapDensity * 10.0f * distanceToDatum;

                                //seaweight at datum depth
                                var waterHeight = offshoreFlag * (p.Z + halfCellHeight + distanceToDatum);  //this is the sea level. pz is negative
                                overburden += (waterHeight < 0.0f ? (float)(-1.0f * 10.0 * seaWaterDensity * waterHeight) : 0.0f);
                                values[counter] = overburden;
                                counter += 1;
                            }
                        }
                    }
        ...
        public static string WriteBoundaryConditions( BoundaryConditionsItem bcItem, MaterialsModelItem matItem, string simulationName, string caseFolder, SeismicCube referenceCube )//, ref List<string> filesToInclude)
        {
            ...
            configSections += WriteEdgeLoadsFile(bcItem, matItem, simulationName, caseFolder, referenceCube);//, ref List<string> filesToInclude)
        ...
        public static string WriteEdgeLoadsFile( BoundaryConditionsItem bcItem, MaterialsModelItem matItem, string simulationName, string caseFolder, SeismicCube referenceCube )//, ref List<string> filesToInclude)
        {
            // SimulationModelItem simObject = gigaModel.SimulationsModel.GetOrCreateModel(simulationName);
            BoundaryConditionsItem bc = bcItem;// gigaModel.BoundaryConditionsModels.GetOrCreateModel(simObject.BoundaryConditionsModelName);
            SeismicCube densityCube = (SeismicCube)(DataManager.Resolve(new Droid(matItem.Density.DroidString)));
            ...
            //lets start with the edge load.
            Object datumObject = null;
            Droid droid = Droid.TryParse(bc.DatumDroid);
            if (droid != Droid.Empty)
            {
                datumObject = DataManager.Resolve(droid);
            }
            else
                datumObject = bc.DatumValue;

            //Object datumObject = droid == Droid.Empty ? 0.0f : DataManager.Resolve(droid);
            float[] edgeLoads = GetEdgeLoad(datumObject, bc.Offshore, bc.GapDensity, bc.SeaWaterDensity, densityCube);

            //write the edgeLoads as a single one-chunk binary object. This function can be async.
            //string edgeLoadFileName = simulationName + ".edg";
            string edgeLoadFileName = Path.Combine(caseFolder, simulationName + ".edg");

            using (System.IO.StreamWriter w = new StreamWriter(edgeLoadFileName))
            {
                //*EDGELOADS,NRM
                w.WriteLine("*EDGELOADS");
                w.WriteLine(densityCube.NumSamplesIJK.I * densityCube.NumSamplesIJK.J);
                int computedValuecounter = 0;
                int elementCounter = 0;
                int k = 0;
                int[] Cells = dims.Cells;
                float[] p = edgeLoads.Select(t => (float)(t * 0.001)).ToArray();
                for (int i = 0; i < densityCube.NumSamplesIJK.I; i++)
                {
                    for (int j = 0; j < densityCube.NumSamplesIJK.J; j++)
                    {
                        for (int nk = 0; nk < densityCube.NumSamplesIJK.K; nk++)
                        {
                            if (nk == 0)
                            {
                                int nodeAIndex = GetFirstNodeOfCell(k, j, i, Cells);
                                int nodeBIndex = nodeAIndex + (Cells[2] + 1);
                                int nodeCIndex = nodeAIndex + ((Cells[2] + 1) * (Cells[1] + 1));
                                int nodeDIndex = nodeCIndex + (Cells[2] + 1);

                                w.WriteLine((1 + elementCounter).ToString() + "  4 ");
                                w.WriteLine(nodeAIndex + "  " + nodeBIndex + "  " + nodeDIndex + "  " + nodeCIndex);
                                w.WriteLine("0.0    0.0     0.0    0.0");
                                w.WriteLine("0.0    0.0     0.0    0.0");
                                w.WriteLine(p[computedValuecounter] + "  " + p[computedValuecounter] + "  " + p[computedValuecounter] + "  " + p[computedValuecounter]);
                                computedValuecounter += 1;
                            }
                            elementCounter += 1;
                        }
                    }
        ...
        private static int GetFirstNodeOfCell( int cellKIndex, int cellJIndex, int cellIIndex, int[] Cells )
        {
            return 1 + cellKIndex + cellJIndex * (Cells[2] + 1) + cellIIndex * ((Cells[2] + 1) * (Cells[1] + 1));
        }
        ...
        public static string WriteDisplacementsFile( BoundaryConditionsItem bcItem, MaterialsModelItem matItem, string simulationName, string caseFolder, SeismicCube referenceCube )//, ref List<string> filesToInclude)
        {
            ...
            if (IProjectedNorth >= JProjectedNorth) //then I is pointing north. It shoould get the max strain if angle is zero and min strain if 90. This is the FIVESIZSEVEN Example
            {
                ...
                //version that I thought was correct
                //visage direction.
                ...
                //simply change j and  i
                visageDirectionForDisplacementI = 1;
                visageDirectionForDisplacementJ = 3;
                signI = 1.0;
                //var swapp = displacementI; displacementI = displacementJ;displacementJ = swapp;
            }
            else
            {
                ...
                //version of the code that I thought was right but apparently not.
        ...
        private static void writeElementalDisplacement( System.IO.StreamWriter file, int element, int direction, double displacement )
        {
        ...
        public static List<string> WriteDeckCompressedBinary( SimulationModelItem item, GigaModelDataModel model, bool UseStreams, out bool error, Dictionary<string, List<KeyValuePair<string, bool>>> options = null )// Slb.Ocean.Petrel.IProgress bar = null)
        {
            error = false;
            //int progress = 0;
            ...
            try
            {
                ...
                Slb.Ocean.Petrel.PetrelLogger.Status(new string[] { "Processing boundary conditions " });
                configSections += (WriteBoundaryConditions(bcItem, mats, simulationName, caseFolder["BoundariesFolder"], referenceCube));//, ref theFilesToIncludeInTheMII));
                                                                                                                                         // if (bar != null) bar.ProgressStatus = 10;
                Slb.Ocean.Petrel.PetrelLogger.Status(new string[] { "Processing property model " });
                configSections += (WriteMatFiles(mats, simulationName, caseFolder["MaterialsFolder"]));

                //if (bar != null) bar.ProgressStatus = 40;
                //System.Windows.Forms.Application.DoEvents();
                Slb.Ocean.Petrel.PetrelLogger.Status(new string[] { "Processing pressures" });
                configSections += (WritePressures(pressures, simulationName, caseFolder["PressuresFolder"]));//, ref theFilesToIncludeInTheMII));
                //if (bar != null) bar.ProgressStatus = 90;

                //System.Windows.Forms.Application.DoEvents();

                Slb.Ocean.Petrel.PetrelLogger.Status(new string[] { "Creating config files" });
                WriteConfigFiles(configSections, caseFolder["CaseFolder"], simulationName);//, ref theFilesToIncludeInTheMII);
                ...
                if (options != null)
                    mii.UpdateSwitches(options);

                Slb.Ocean.Petrel.PetrelLogger.Status(new string[] { "Creating MII" });
