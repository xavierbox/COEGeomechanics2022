...
namespace Gigamodel.OceanUtils
{
    public class BoreholeTools
    {
        ...
        public static void ReplaceLog(ref WellLog log, List<float> values, List<float> mds, bool sort = true)
        {
            ...
            for (int k = 0; k < Math.Min(valuesCorrected.Count, 10); k++)
                PetrelLogger.InfoOutputWindow(log.Name + " --Sorted samples after sorting are " + valuesCorrected[k].Value);
            ...
            try
            {
                ...
                using (var trans = DataManager.NewTransaction())
                {
                    ...
                    trans.Lock(log);
                    trans.Lock(log.Borehole);
                    trans.Lock(log.WellLogVersion);
                    trans.Lock(log.LogCollection);
                    log.Samples = valuesCorrected;
                    //log.SetSamples(0.0, valuesCorrected);
        ...
        public static void getTrajectoryPoints(Borehole w, List<Point3> pts, List<double> mds, double spacing = 0.5 * 0.30428)
        {
            pts.Clear();// = new List<Point3>();
            mds.Clear();// = new List<double>();

            double minMd = w.MDRange.Min, maxMd = w.MDRange.Max;
            //double spacing = 0.5 * 0.30428;
            double md = minMd;
            while (md < maxMd)
            {
    ...
    {
        public static SplitCubeFloats splitBinaryCubeOLD(SeismicCube cube, float scalingFactor = 1.0f)
        {
            //memory allocation
        ...
        public static SplitCubeFloats splitBinaryCube(SeismicCube cube, float scalingFactor = 1.0f)
        {
            ...
            //copy the data from seismic traces to here
            float[] floatsSingleTrace = new float[cube.NumSamplesIJK.K];
            int traceCounter = 0;
        ...
        public static List<SeismicCube> XFileToSeismicCubes(SeismicCube refCube, List<KeywordDescription> propList, string eclipseFileName, List<Template> templates)
        {
            ...
            foreach (KeywordDescription propDes in propList)
            {
                ...
                if (c != null)
                {
                    long cubesize = c.NumSamplesIJK.I * c.NumSamplesIJK.J * c.NumSamplesIJK.K;
                    if (propDes.Size == cubesize)
                    {
            ...
            using (ITransaction txn = DataManager.NewTransaction())
            {
                ...
                for (int i = 0; i < c.NumSamplesIJK.I; i++)
                {
                    for (int j = 0; j < c.NumSamplesIJK.J; j++)
                    {
                        ...
                        ITrace targetTrace = c.GetTrace(i, j);// i,j);//, j);

                        //var ttt = c.NumSamplesIJK;
        ...
        public static void SeismicToLog(SeismicCube s, Borehole w, double spacing = -1)
        {
            List<Point3> thisWellPoints = new List<Point3>();
            List<double> mds = new List<double>();

            if (spacing < 0) //take it from seismic
            {
                spacing = 0.5 * Math.Min(s.SampleSpacingIJK.X, Math.Min(s.SampleSpacingIJK.Y, s.SampleSpacingIJK.Z));
            }

            BoreholeTools.getTrajectoryPoints(w, thisWellPoints, mds, spacing);
            float[] traceYMValues = new float[s.NumSamplesIJK.K];
            Index2 oldTrace = new Index2(-999, -999);

            List<float> values = new List<float>();
            foreach (Point3 p in thisWellPoints)
            {
                IndexDouble3 trace3D = s.IndexAtPosition(p); // pointMapToTrace3D[p];
                Index2 trace2D = new Index2((int)trace3D.I, (int)trace3D.J);

                float value = float.NaN;
                if ((trace3D.K >= 0) && (trace3D.I > 0) && (trace3D.J > 0) && (trace3D.I < s.NumSamplesIJK.I) && (trace3D.J < s.NumSamplesIJK.J) && (trace3D.K < s.NumSamplesIJK.K))
                {
                    //if we got the trace before and the new point is in the same trace, dont load it again for efficiency
                    //example: vertical wells would have always the same trace.
                    if (trace2D != oldTrace)
                    {
                        oldTrace = new Index2(trace2D.I, trace2D.J);
                        s.GetTraceData(trace2D.I, trace2D.J, traceYMValues);
                    }

                    value = traceYMValues[(int)(trace3D.K)];
                }
            ...
            var t = s.Template;
            var l1 = BoreholeTools.GetOrCreateLog(intendedName: s.Name, well: w, templateInput: t);

            //if the values expected in this well log template are floats
            BoreholeTools.ReplaceLog(log: ref l1, values: values, mds: mds.Select(x => (float)(x)).ToList(), sort: false);
        ...
        public static bool WellPoroelasticStressFromSeismic(Object datum, bool offshore, float gapDensity, float seaWaterDensity,  SeismicCube ym, SeismicCube pr, SeismicCube dens, SeismicCube press, List<Borehole> wells, float emin, float emax, float offset, string name = "PoroelasticIsotropic")
        {
            ...
            foreach (Borehole w in wells)
            {
                ...
                Template t = PetrelProject.WellKnownTemplates.GeomechanicGroup.StressTotal;
                WellLog l1 = BoreholeTools.GetOrCreateLog("SVertical", w, t, name);
                WellLog l2 = BoreholeTools.GetOrCreateLog("Shmin", w, t, name);
                WellLog l3 = BoreholeTools.GetOrCreateLog("Shmax", w, t, name);

                t = PetrelProject.WellKnownTemplates.MiscellaneousGroup.General;
                WellLog l4 = BoreholeTools.GetOrCreateLog("ShRatio", w, t, name);
                WellLog l5 = BoreholeTools.GetOrCreateLog("Regime", w, t, name);

                List<float> mdsFloats = mds.Select(x => (float)(x)).ToList();
                BoreholeTools.ReplaceLog(ref l1, svLogValues, mdsFloats, false);
                BoreholeTools.ReplaceLog(ref l2, shminLogValues, mdsFloats, false);
                BoreholeTools.ReplaceLog(ref l3, shmaxLogValues, mdsFloats, false);

                List<float> shRatioValues = new List<float>();

                for (int i = 0; i < shmaxLogValues.Count; i++)
                {
                    shRatioValues.Add(shmaxLogValues[i] / shminLogValues[i]);
                    //shmaxLogValues[i] /= shminLogValues[i];
                }
                BoreholeTools.ReplaceLog(ref l4, shRatioValues, mdsFloats, false);
                BoreholeTools.ReplaceLog(ref l5, stressRegime, mdsFloats, false);
            } //borehole
