...
public enum DataFormatTypesEnum
{
...
class EclipseReader
{
    ...
    public static bool LoadFloatsAsSplitCube(KeywordDescription prop, string fileToParse, ref SplitCubeFloats data)
    {
        ...
        for (int n = 0; n < prop.Size; n++)
        {
            ...
            floatsArray[n] = BitConverter.ToSingle(dataArray, 0);
...
/*
               int bytesReadBruteForce = 0;
               int interruptions = 0;
               for (int n = 0; n < key.Size; n++)
               {
                   reader.ReadBytes( key.WordSize );
                   bytesReadBruteForce += key.WordSize;
                   if ((n > 0) && (n%1000==0))
                   {
                       reader.ReadBytes(8);
                       bytesReadBruteForce += 8;
                       interruptions += 1;
                   }
               }
               reader.ReadBytes(4);
               */
//reader.BaseStream.Seek(currentLocation + toSkip, SeekOrigin.Begin);
