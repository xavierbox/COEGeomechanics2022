...
using System.Threading.Tasks;
    ...
    public class SplitCubeFloats : SplitCube<float>
    {
        ...
        public SplitCubeFloats(int ni, int nj, int nk, int maxFloatsPerChunk = 200000000) : base()
        {
            Split(ni, nj, nk, maxFloatsPerChunk);
        }
        ...
        public void Split(int ni, int nj, int nk, int maxFloatsPerChunk = 200000000)
        {
            //first guess a reasonable split for floats 
            int tracesPerChunk = Math.Min(ni * nj, (int)(0.75 * maxFloatsPerChunk / nk));
            tracesPerChunk = Math.Max(1, tracesPerChunk); //this is needed for maxFloatsPerChunk very small 

            int nChunks = ni * nj / (tracesPerChunk);
            ...
            int lastChunkTraces = ni * nj - (nChunks - 1) * tracesPerChunk;
            ...
            _parts[nChunks - 1] = new float[(ni * nj - tracesPerChunk * (nChunks - 1)) * nk];
